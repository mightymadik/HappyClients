...
class HomeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let subButton: UIButton = {
        ...
        button.backgroundColor = .systemBlue
        button.tintColor = .white
        button.setImage(UIImage(systemName: "dollarsign.square.fill",
                                withConfiguration: UIImage.SymbolConfiguration(pointSize: 32, weight: .medium)),
                        for: .normal)
        button.layer.cornerRadius = 30
        button.layer.shadowColor = UIColor.label.cgColor
    ...
    override func viewDidLoad() {
        ...
        subButton.addTarget(self, action: #selector(didTapSub), for: .touchUpInside)
    ...
    override func viewDidLayoutSubviews() {
        ...
        subButton.frame = CGRect(
            x: view.frame.width - 80 - 310,
            y: view.frame.height - 88 - view.safeAreaInsets.bottom,
            width: 60,
            height: 60
        )
